    ...
    <AppLayout>
        <div class="py-6">
            <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="mb-6">
                    <Link :href="`/agents/${agent.id}`" class="text-sm text-indigo-600 hover:text-indigo-800 mb-2 inline-block">
                        ← 返回 Agent 详情
                    </Link>
                    <h1 class="text-2xl font-bold text-gray-900">编辑 Agent</h1>
                    <p class="mt-1 text-sm text-gray-500">修改 #{{ agent.id }} {{ agent.name }} 的配置</p>
                </div>
                ...
                <form @submit.prevent="submit" class="bg-white rounded-xl shadow-sm border border-gray-200">
                    <div class="p-4 sm:p-6 space-y-5 sm:space-y-6">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-400">1-100</p>
                        </div>
...
const form = reactive({
    ...
    max_capacity: props.agent.max_capacity || 5,
})
