    ...
    <AppLayout>
        <div class="py-6">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <!-- Flash 消息 -->
                <div v-if="$page.props.flash?.success" class="max-w-7xl mx-auto mt-6">
                ...
                <div class="mt-6 grid grid-cols-4 gap-4">
                    <div v-for="stat in stats" :key="stat.key" class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 text-center">
                ...
                <div class="mt-6 space-y-6">
                    <div v-for="group in statusGroups" :key="group.status" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        <!-- 分组标题 -->
                        <div class="px-6 py-3 border-b border-gray-100 flex items-center justify-between" :class="group.headerBg">
                            <div class="flex items-center space-x-2">
                                <span class="w-3 h-3 rounded-full" :class="group.dotColor"></span>
                                <h2 class="text-base font-semibold text-gray-900">{{ group.label }}</h2>
                                <span class="text-sm text-gray-400">({{ group.tasks.length }})</span>
                            </div>
                            <div v-if="group.tasks.length > 0" class="flex items-center space-x-2">
                                <select
                                    v-model="group.typeFilter"
                                    @change="filterGroup(group)"
                                    class="text-xs border border-gray-200 rounded px-2 py-1 bg-white focus:outline-none focus:ring-1 focus:ring-indigo-400"
                                >
                        ...
                        <div v-if="group.filteredTasks.length > 0">
                            <div
                                ...
                                :key="task.id"
                                class="px-6 py-4 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors"
                            >
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-3 min-w-0">
                                        <span class="text-lg flex-shrink-0">{{ typeIcon(task.type) }}</span>
                                        <div class="min-w-0">
                                            <Link :href="`/tasks/${task.id}`" class="text-sm font-medium text-gray-900 hover:text-indigo-600">
                                                {{ task.title }}
                                            </Link>
                                    ...
                                    <span class="text-xs text-gray-400 flex-shrink-0 ml-4">{{ task.created_at }}</span>
                                </div>
                            </div>
                        </div>
